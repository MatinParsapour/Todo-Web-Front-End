...
export class InsertListComponent implements OnInit {
  ...
  addList() {
    ...
    this.insertListService
      .update('/list/add-list-to-folder', this.listDTO.value)
      .subscribe(
        (response: any) => {
          this.closeDialog();
          this.notifier.notify(
            NotificationType.SUCCESS,
            'The list succeccfully added'
          );
        ...
        (error: HttpErrorResponse) => {
          this.notifier.notify(
            ...
            'Something went wrong, most likely name of the list is doplicate'
          );
          this.isLoading = false;
        }
      );
